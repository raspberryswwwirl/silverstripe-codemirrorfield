...
function instanceWatch(source, value, options) {
  ...
  const cur2 = currentInstance;
  setCurrentInstance(this);
  const res = doWatch(getter, cb.bind(publicThis), options);
  if (cur2) {
    setCurrentInstance(cur2);
  } else {
...
function createPathGetter(ctx, path) {
  ...
  return () => {
    let cur2 = ctx;
    for (let i = 0; i < segments.length && cur2; i++) {
      cur2 = cur2[segments[i]];
    }
    return cur2;
  };
}
...
const templateContainer = doc$1 && doc$1.createElement("template");
const nodeOps = {
...
class ViewPlugin {
  ...
  static define(create, spec) {
    const { eventHandlers, provide: provide2, decorations: deco } = spec || {};
...
class MatchDecorator {
  constructor(config2) {
    ...
    if (decorate) {
      ...
      this.addMatch = (match, view, from, add2) => add2(from, from + match[0].length, getDeco(match, view, from));
    } else {
...
export { EditorView as E, onMounted as a, onBeforeUpdate as b, computed as c, onUpdated as d, renderSlot as e, basicSetup as f, defineCustomElement as g, onBeforeMount as o, ref as r };
